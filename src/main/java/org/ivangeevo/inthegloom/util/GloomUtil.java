...
public class GloomUtil {
    ...
    public static boolean isInGloom(PlayerEntity player)
    {
        ...
        if (!player.isCreative() && !player.hasStatusEffect(StatusEffects.NIGHT_VISION)) {
            ...
            // Compute the sun brightness with moon phases
            float sunBrightness = computeOverworldSunBrightnessWithMoonPhases(world);

            // If there's no sunlight (during night)
            if (sunBrightness < 0.02D) {
                // Check light levels at player's position and above
                float blockLightValue = world.getLightLevel(LightType.BLOCK, pos);
                float blockAboveLightValue = world.getLightLevel(LightType.BLOCK, pos.up());

                // Get the higher light value between current and above block
                float effectiveLightValue = Math.max(blockLightValue, blockAboveLightValue);

                // Only return true if block light levels are extremely low
                return effectiveLightValue < 0.001F; // Gloom condition based on block light levels
            }
        }
    ...
    private static float computeOverworldSunBrightnessWithMoonPhases(World world)
    {
        ...
        sunInvertedBrightness = MathHelper.clamp(sunInvertedBrightness, 0.0F, 1.0F); // Clamp between 0 and 1
        ...
        sunBrightness *= rainBrightnessModifier * stormBrightnessModifier; // Adjust brightness
        ...
        if (minBrightness < 0.05D) {
            minBrightness = 0D; // Clamp minimum brightness
        }

        return (float) (sunBrightness * (1D - minBrightness) + minBrightness); // Final brightness value
    }
}
